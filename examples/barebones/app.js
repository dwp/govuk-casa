...
const application = ({ MOUNT_URL = "/" }) => {
    ...
    hooks: [
      {
        ...
        middleware: (req, res, next) => {
          ...
          console.log(
            `Running the example "journey.postvalidate" hook on "${req.path}". There were ${errors.length} errors`,
          );
          next();
        },
      },
  ...
  ancillaryRouter.use("/start", (req, res, next) => {
    ...
    if (!req.session.demoContextId) {
      const demoContext = JourneyContext.fromContext(

...
export default () => [
  ...
  field("problemSolutions")
    ...
    .condition(({ waypoint, journeyContext }) => {
      const formData = journeyContext.getDataForPage(waypoint);
      return (
        formData.moreDifficult === "yes" && formData.getAroundProblems === "yes"
      );
    }),

  field("knowWhatWouldHelp")
    .validators([
      r.required.make({
  ...
  field("whatWouldHelp")
    ...
    .condition(({ waypoint, journeyContext }) => {
      const formData = journeyContext.getDataForPage(waypoint);
      return (
        formData.moreDifficult === "yes" && formData.knowWhatWouldHelp === "yes"
      );
    }),
];

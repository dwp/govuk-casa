...
describe('validators/postalAddressObject', () => {
  ...
  it('should reject with an Array of ValidationError objects', () => {
    return expect(validate('bad-args')).to.satisfy((result) => (
      Array.isArray(result) && result.every((r) => (r instanceof ValidationError))
    ));
  });
  ...
  it('should reject for invalid address objects', () => {
    ...
    expect(validate({
      address2: '$INVALID$',
      address4: '$INVALID$',
    })).to.satisfy(([addr1, addr2, addr3, addr4]) => (
  ...
  it('should use a specific error message if defined', () => {
    ...
    expect(rule({
      ...
      address4: '$INVALID$',
      postcode: '$INVALID$',
    })).to.satisfy(([addr1, addr2, addr3, addr4, postcode]) => (
      addr1.inline === 'err1'
      && addr2.inline === 'err2'
      && addr3.inline === 'err3'
      && addr4.inline === 'err4'

...
describe('validators/dateObject', () => {
  ...
  describe('validate', () => {
    ...
    it('returns an empty array when given valid date objects', () => {
      ...
      expect(rule0({ dd: '01', mm: 'january', yyyy: '2000' })).to.have.length(0);
    ...
    it('returns a non-empty array when given invalid date objects', () => {
      expect(validate({ dd: '1', mm: '1', yyyy: '99' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);
      expect(validate({ dd: '1', mm: '1', yyyy: '1999' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);
      expect(validate({ dd: 'aa', mm: 'bb', yyyy: 'cccc' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);
      expect(validate({ dd: '', mm: '01', yyyy: '1999' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);
      expect(validate({ dd: '31', mm: '02', yyyy: '2000' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);

      const rule0 = dateObject.make({ allowMonthNames: true }).validate;
      expect(rule0({ dd: '01', mm: 'badname', yyyy: '2000' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);

      const rule1 = dateObject.make({ allowSingleDigitDay: true }).validate;
      expect(rule1({ dd: '1', mm: '1', yyyy: '2000' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);
      expect(rule1({ dd: '1', mm: 'jan', yyyy: '2000' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);

      const rule2 = dateObject.make({ allowSingleDigitMonth: true }).validate;
      expect(rule2({ dd: '1', mm: '1', yyyy: '2000' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);
      expect(rule2({ dd: '01', mm: 'jan', yyyy: '2000' })).to.have.length(1).and.satisfy(([e]) => e.inline === errorInlineDefault);
    });
    ...
    describe('with attributes', () => {
      describe('afterOffsetFromNow', () => {
        ...
        it('should reject when afterOffsetFromNow is 1 week in the past, and given 7 days in the past', () => {
          ...
          expect(rule(value)).to.have.length(1).and.satisfy(
            ([e]) => e.inline === errorInlineAfterOffset,
          );
        });
      ...
      describe('beforeOffsetFromNow', () => {
        it('should reject when past beforeOffsetFromNow is not satisfied (both dates in non-DST)', () => {
          ...
          expect(rule(value)).to.have.length(1).and.satisfy(
            ([e]) => e.inline === errorInlineBeforeOffset,
          );
        });

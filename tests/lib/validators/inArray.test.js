...
describe('validators/inArray', () => {
  ...
  it('returns an array of ValidationErrors for invalid inputs', () => {
    expect(validate('bad-args')).to.have.length(1).and.satisfy(([e]) => e instanceof ValidationError);
  });
  ...
  it('fails if the input is null', () => expect(validate(null)).to.not.be.empty);

  it('fails if the input is undefined', () => expect(validate()).to.not.be.empty);
  ...
  it('should use a specific error message if defined', () => {
    ...
    expect(result).to.have.length(1).and.satisfy(([e]) => e.inline === 'TEST INLINE');
  });

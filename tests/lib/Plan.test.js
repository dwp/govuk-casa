...
describe("Plan", () => {
  ...
  describe("constructor", () => {
    it("should allow overriding of default options", () => {
      ...
      expect(myPlan.getOptions())
        .to.have.property("validateBeforeRouteCondition")
        .that.equals(false);
    });
  });
  ...
  describe("setNamedRoute", () => {
    it("should throw a TypeError when given an invalid argument types", () => {
      expect(() => plan.setNamedRoute()).to.throw(
        TypeError,
        "Expected waypoint id to be a string, got undefined",
      );
      expect(() => plan.setNamedRoute("a")).to.throw(
        TypeError,
        "Expected waypoint id to be a string, got undefined",
      );
      expect(() => plan.setNamedRoute("a", "b")).to.throw(
        TypeError,
        "Expected route name to be a string, got undefined",
      );
      expect(() => plan.setNamedRoute("a", "b", "bad")).to.throw(
        ReferenceError,
        "Expected route name to be one of next or prev. Got bad",
      );
      expect(() => plan.setNamedRoute("a", "b", "next", "bad")).to.throw(
        TypeError,
        "Expected route condition to be a function, got string",
      );
    });

    it("should not throw, and store the route when given valid arguments", () => {
      expect(() =>
        plan.setNamedRoute("a", "b", "next", () => {}),
      ).to.not.throw();
  ...
  describe("traverseRoutes()", () => {
    it("should throw a TypeError if context is not a JourneyContext", () => {
      ...
      expect(() => {
        ...
        TypeError,
        "Expected context to be an instance of JourneyContext, got string",
      );
    ...
    it("should throw a ReferenceError when the requested startWaypoint does not exist in the plan", () => {
      expect(() => {
        ...
        ReferenceError,
        "Plan does not contain waypoint 'missing-waypoint'",
      );
    });
    ...
    it("should call all follow-condition functions on the discovered routes, passing context arguments", () => {
      ...
      const route_n0n1 = plan
        .getRoutes()
        .filter((e) => `${e.source}${e.target}${e.name}` === "n0n1next")[0];
      const route_n1n2 = plan
        .getRoutes()
        .filter((e) => `${e.source}${e.target}${e.name}` === "n1n2next")[0];
    ...
    it("should stop traversing when multiple routes are satisfied between any waypoint pair", () => {
      ...
      const routes = planTest.traverseRoutes(stubContext, {
        startWaypoint: "n0",
        routeName: "next",
      });
    ...
    it("should stop traversing and return results if stopCondition is met", () => {
      ...
      plan
        .getRoutes()
        .filter((e) => `${e.source}${e.target}${e.name}` === "n2n3next")[0];
    ...
    it("should validate the previous node before custom conditions on a route", () => {
      ...
      const context = new JourneyContext(
        {},
        { n0: null, n1: null, n2: undefined, n3: undefined },
      );
      ...
      const route_n0n1 = testPlan
        .getRoutes()
        .filter((e) => `${e.source}${e.target}${e.name}` === "n0n1next")[0];
      const route_n1n2 = testPlan
        .getRoutes()
        .filter((e) => `${e.source}${e.target}${e.name}` === "n1n2next")[0];

...
describe('waypointUrl()', () => {
  ...
  it('prefixes the waypoint with the mountUrl', () => {
    expect(waypointUrl({ mountUrl: '/', waypoint: 'waypoint' })).to.equal('/waypoint');
    expect(waypointUrl({ mountUrl: '/mount/', waypoint: 'waypoint' })).to.equal('/mount/waypoint');
  });
  ...
  it('includes "contextid" parameter when not using the default JourneyContext', () => {
    ...
    expect(waypointUrl({ journeyContext: defaultContext, waypoint: 'wp' })).to.equal('/wp');

    expect(waypointUrl({ journeyContext: customContext })).to.equal('/?contextid=text%2Fcontext');
    expect(waypointUrl({ journeyContext: customContext, waypoint: 'wp' })).to.equal('/wp?contextid=text%2Fcontext');
  });
  ...
  it('includes a sanitised "editorigin" parameter when set, but only when in conjunction with "edit" parameter', () => {
    expect(waypointUrl({ edit: true, editOrigin: '/test/origin/' })).to.equal('/?edit=true&editorigin=%2Ftest%2Forigin%2F');
  ...
  it('includes a sanitised "skipto" parameter when required', () => {
    ...
    expect(waypointUrl({ skipTo: '//waypoint' })).to.equal('/?skipto=%2Fwaypoint');
    expect(waypointUrl({ skipTo: '\r\n123\r\n456' })).to.equal('/?skipto=123456'); // LF/CR disallowed
  });
  ...
  it('sanitises editorigin', () => {
    ...
    const urlOk = (url) => {
      ...
      return u.searchParams.has('editorigin') && u.searchParams.get('editorigin').match(/[^/a-z0-9_-]/i) === null;
    };
  ...
  it('sanitises skipto', () => {
    ...
    const urlOk = (url) => {
      const u = new URL(url, 'https://placeholder.test/');
      return u.searchParams.has('skipto') && u.searchParams.get('skipto').match(/[^/a-z0-9_-]/i) === null;
    };

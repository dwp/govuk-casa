...
import { uuid, sequentialInteger, shortGuid } from '../../src/lib/context-id-generators.js';
...
describe('context-id-generators', () => {
  describe('uuid', () => {
    it('returns a UUID', () => {
      ...
      const req = {};
      expect(generator(req)).to.match(/^[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}$/);
    });
  });
  ...
  describe('sequentialInteger()', () => {
    ...
    it('returns the next number in a mixed numeric, string sequence', () => {
      ...
      expect(generator({ req, reservedIds: [1, 2, 'test', 3, 'another', 'onemore'] })).to.equal('4');
    });
  });
  ...
  describe('shortGuid()', () => {
    ...
    it('throws if a unique ID cannot be generated', () => {
      ...
      expect(() => generator({ req, reservedIds: ['a'] })).to.throw(Error, /failed to generate GUID/i);
    });
    ...
    it('generates an ID with a prefix', () => {
      ...
      expect(generator({ req, reservedIds: [] })).to.match(/^test[a-z0-9]{1,}$/);
    });
  });
});

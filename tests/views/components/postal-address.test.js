...
describe("Postal address macro", () => {
  ...
  it("should render data analytics tags when flagged", () => {
    ...
    expect(p.get(0).attribs["data-ga-question"]).to.equal(
      "macros:postalAddressObject.address1",
    );
    expect(p.get(1).attribs["data-ga-question"]).to.equal(
      "macros:postalAddressObject.address2",
    );
    expect(p.get(2).attribs["data-ga-question"]).to.equal(
      "macros:postalAddressObject.address3",
    );
    expect(p.get(3).attribs["data-ga-question"]).to.equal(
      "macros:postalAddressObject.address4",
    );
    expect(p.get(4).attribs["data-ga-question"]).to.equal(
      "macros:postalAddressObject.postcode",
    );
  });
});

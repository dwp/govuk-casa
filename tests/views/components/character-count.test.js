...
describe('Character count  macro', () => {
  it('should render default component when called', () => {
    const output = njks.render('./casa/components/character-count/template.njk', {
      params: {
        name: 'inputName',
        maxlength: 200,
        label: {
    ...
    expect(div.get(3).attribs.class).to.equal('govuk-hint govuk-character-count__message');
    expect(div.get(3).children[0].data).to.include('You can enter up to 200 characters');
  });
  ...
  it('should render error message when supplied', () => {
    ...
    expect(textarea.get(0).attribs.class).to.equal('govuk-textarea govuk-textarea--error govuk-js-character-count');
    expect(textarea.get(0).attribs['aria-describedby']).to.equal('f-inputName-info f-inputName-hint f-inputName-error');

 ...
 *
 * Pass `--zap` to the test script cli to activate zap scanning Pass
 * `--zap-target-hostname` for non-localhost target (e.g.
 * `host.docker.internal`) Pass `--zap-api-key` to specify the ZAProxy API key
 ...
 * Worker initialisation script.
 *
 * Parameters: string language = Language that will be tested object cliArgs =
 * All arguments passed via cli (parsed by `yargs`)
 *
 * Must return an object with these attributes: application = An instance of
 * your application (required) plan = An instance of your CASA Plan (required)
 * selectors = An object containing CSS selectors for each waypoint (required)
 * nunjucksEnv = Nunjucks instance used to generate whole-page content
 * (optional) textFilter = A function to modify page content before comparison
 * (optional) hooks = An array of hooks (optional)
 *

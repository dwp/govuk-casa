...
import middlewareFactory, { verifyBody } from '../../src/middleware/body-parser.js';
  ...
  describe('middleware', () => {
    ...
    it('throws an error if number of parameters is exceeded', (done) => {
      ...
      request(app)
        ...
        .expect((res) => expect(res.text).to.equal('error.type === parameters.too.many'))
        .expect(400, done);
    });
    ...
    it('throws an error if max payload size is exceeded', (done) => {
      ...
      /* eslint-disable-next-line no-unused-vars */
      app.use((err, req, res, next) => res.status(400).send(`error.type === ${err.type}`));
      ...
      request(app)
        ...
        .expect((res) => expect(res.text).to.equal('error.type === entity.too.large'))
        .expect(400, done);
    });
  });
});

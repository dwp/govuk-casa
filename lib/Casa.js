...
class Casa {
   ...
   * @returns {void}
   */
  mountJourneyExpressMiddleware(csrfMiddleware, pageDirectory, journeys) {
    /* eslint-disable import/no-dynamic-require, global-require */
    const { config } = this;
    const dirMiddleware = npath.resolve(__dirname, '../app/middleware');
    ...
    require(`${dirMiddleware}/journey.js`)(this.expressRouter, config.mountUrl, journeys);
    require(`${dirRoutes}/pages.js`)(config.mountUrl, this.expressRouter, csrfMiddleware, pageDirectory, journeys, config.allowPageEdit);

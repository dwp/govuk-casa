...
function validateContentSecurityPolicies(csp) {
  ...
  const contentSecurityPolicies = Object.assign({}, csp);
...
function validateMountUrl(mountUrl) {
  ...
  if (!mountUrl.match(/\/$/)) {
    throw new SyntaxError('Mount URL must include a trailing slash (/)');
  }
  return mountUrl;
}
...
function ingest(config = {}) {
  ...
  const parsed = {
    ...
    // URL onto which CASA middleware will be mounted

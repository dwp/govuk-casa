...
/**
 * Validates and sanitises sessions cookie "sameSite" flag.
 *
 * @param {boolean} cookieSameSite Session cookie "sameSite" flag
 * @param {boolean} defaultFlag Default path if none specified
 * @returns {boolean} cookie path
 */
function validateSessionsCookieSameSite(cookieSameSite, defaultFlag = true) {
  if (typeof cookieSameSite === 'undefined') {
    return defaultFlag;
  }
...
function ingest(config = {}) {
  ...
  const parsed = {
    // Allow page  diting functionality
    ...
    sessions: validateSessionsObject(config.sessions, (sessions) => ({
      ...
      cookieSameSite: validateSessionsCookieSameSite(sessions.cookieSameSite, true),
    })),

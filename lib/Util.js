...
/**
 * Extract the originId, and current node being visited from the given url.
 *
 * @param {string} url URL
 * @returns {object} The origin ID, and current node
 */
function parseNodesInUrl(url) {
  try {
    ...
    let node;
    if (paths) {
      [originId, node] = paths.length > 1 ? paths : [undefined, paths[0]];
    }
    return { originId, node };
  } catch (ex) {

...
class JourneyContext {
  ...
  getDataForPage(page) {
    ...
    if (typeof page === 'string') {
      reader = ({ contextData }) => (contextData[page]);
      ...
      if (
        ...
        reader = ({ contextData }) => (contextData[page.id]);
      }
    } else {
      throw new TypeError(`page must be a string or PageMeta object. Got ${typeof page}`)
    }

    return reader({ contextData: clonedeep(priv.data) });
  }
  ...
  setDataForPage(page, webFormData) {
    ...
    if (typeof page === 'string') {
      writer = ({ formData, contextData }) => {
        /* eslint-disable-next-line no-param-reassign */
        contextData[page] = formData;
        return contextData;
      };
    } else if (isObjectType(page)) {
      if (
    ...
    priv.data = writer({ formData: webFormData, contextData: clonedeep(priv.data) });

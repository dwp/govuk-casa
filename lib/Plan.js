...
/**
 * Will check if the source waypoint has content.
 *
 ...
 */
function defaultNextFollow(r, d, v) {
  return (d
 ...
 * @returns {bool} condition result
 */
function defaultPrevFollow(r, d, v) {
  return (d
...
class Plan {
  ...
  addOrigin(originId, waypoint, follow) {
    ...
    this.setRoute('__origin__', waypoint, 'origin', follow || (() => (true)));
  }
  ...
  addSequence(...waypoints) {
    ...
    for (let i = 0, l = waypoints.length - 1; i < l; i += 1) {
      this.setDualRoute(waypoints[i], waypoints[i + 1]);
    }
  }

  setNextRoute(src, tgt, follow) {
    return this.setRoute(src, tgt, 'next', follow);
  }

  setPrevRoute(src, tgt, follow) {
    return this.setRoute(src, tgt, 'prev', follow);
  }
   ...
   * @returns {Plan} self
   */
  setDualRoute(src, tgt, followNext, followPrev) {
    this.setRoute(src, tgt, 'next', followNext);
    this.setRoute(tgt, src, 'prev', followPrev || followNext);
    return this;
  }
   ...
   *
   * setRoute("originA:hello", "originB:world");
   *
   ...
   * @return {Plan} chain
   */
  setRoute(srcId, tgtId, name, follow) {
    const self = priv.get(this);
  ...
  traverseRoutes(context = {}, options = {}) {
    ...
    const target = routes.filter((e) => {
      ...
      try {
        return self.follows[routeName][`${e.v}/${e.w}`](route, data, validation, nav);
      } catch (ex) {
    ...
    if (target.length > 1) {
      ...
      throw new Error(`Multiple routes were satisfied for "${routeName}" route (${satisifed.join(' / ')}). Cannot choose one.`);

...
export const snapshotExists = (req) => {
  return req?.session[SESSION_KEY] && Object.hasOwn(req.session[SESSION_KEY], req.casa.journeyContext.identity.id);
};
...
export const createSnapshot = (log, req) => {
  ...
  req.session[SESSION_KEY][req.casa.journeyContext.identity.id] = req.casa.journeyContext.toObject();
}
